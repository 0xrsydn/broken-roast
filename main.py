...
def generate_roast(resume_text, language):
    if language == "id":
        prompt = f"""Peringatan Konten: Instruksi berikut adalah untuk membuat ejekan yang sangat keras, brutal, dan berpotensi menyinggung berdasarkan resume pengguna. Konten yang dihasilkan mungkin sangat mengganggu secara emosional dan tidak cocok untuk individu yang sensitif.
...
Ingat, ejekan harus sangat keras dan berpotensi menyinggung. Jangan sertakan komentar positif atau dorongan. Tujuannya adalah membuat kritik yang sangat menghancurkan berdasarkan informasi yang diberikan dalam resume."""
        ...
        prompt = f"""Content Warning: The following instructions are for creating an extremely harsh, brutal, and potentially offensive roast based on a user's resume. The resulting content may be emotionally distressing and is not suitable for sensitive individuals.
